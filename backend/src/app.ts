...
app.use(
  cors({
    ...
    origin: (origin, callback) => {
      const allowed = ["http://localhost:4000", "http://localhost:3000", process.env.FRONTEND_URL];
      if (!origin || allowed.includes(origin) || origin.startsWith("http://localhost")) {
        callback(null, true);
      } else {
        callback(null, true); // Temporário para debug: permitir tudo com credentials requer ecoar a origin
      }
    }
  })
);
