...
import PapiService from "../services/PapiService/PapiService";
...
export const show = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (whatsapp.channel === "papi") {
    const qr = await PapiService.getQrCode(whatsapp);
    if (qr) {
      whatsapp.qrcode = qr;
    }
    const statusData = await PapiService.getStatus(whatsapp);
    // Optional: Update status in DB if needed, or just return it
    // whatsapp.status = statusData.status || whatsapp.status;
  }
...
export const remove = async (
  ...
  if (whatsapp.channel === "papi") {
    await PapiService.deleteInstance(whatsapp);
    await DeleteWhatsAppService(whatsappId);
