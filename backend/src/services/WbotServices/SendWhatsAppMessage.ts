...
import PapiService from "../PapiService/PapiService";
...
const SendWhatsAppMessage = async ({
  ...
  if (whatsapp && whatsapp.channel === "papi") {
    ...
    await delay(msdelay);
    const papiResult = await PapiService.sendMessage(whatsapp, {
      jid: number,
      type: "text",
    ...
    const messageData = {
      wid: papiResult.id || papiResult.key?.id || undefined,
      ticketId: ticket.id,
      contactId: undefined,
    ...
    return papiResult as unknown as WAMessage;
  }
