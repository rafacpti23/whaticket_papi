...
export default function TableAttendantsStatus({ loading = false, attendants = [] }) {
    const classes = useStyles();

    function renderList() {
        if (!Array.isArray(attendants)) return null;
        
        return attendants.map((a, k) => (
            <TableRow key={k}>
                <TableCell>{a.name}</TableCell>
                <TableCell align="center">{a.rating || 0}</TableCell>
                <TableCell align="center">{a.countRating || 0}</TableCell>
                <TableCell align="center">{a.tickets || 0}</TableCell>
                <TableCell align="center">{formatTime(a.avgWaitTime || 0)}</TableCell>
    ...
    return (!loading ?
        <TableContainer component={Paper} id="grid-attendants">
            <Table>
                <TableHead>
                    <TableRow>
